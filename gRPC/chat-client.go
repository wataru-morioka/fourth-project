...
func getMessage(c pb.HelloGrpcClient, id string) error {
    ...
    // 無限ループ
    for {
        log.Printf("クライアント：待機")
        // サーバからのメッセージを受信
        msg, err := stream.Recv()
        log.Printf("クライアント：メッセージ取得")
        if err == io.EOF {
            break
        }
...
func main() {
    ...
    if len(os.Args) > 2 {
        switch os.Args[1] {
        case "greet":
            greetServer(ctx, c, os.Args[2])
            if err != nil {
                log.Fatalf("Couldn't execute: %v", err)
            }
        case "add":
            createRoom(ctx, c, os.Args[2])
            if err != nil {
                log.Fatalf("Couldn't execute: %v", err)
            }
        case "get":
            getRoom(ctx, c, os.Args[2])
            if err != nil {
                log.Fatalf("Couldn't execute: %v", err)
            }
        case "list":
            getRooms(ctx, c, os.Args[2])
            if err != nil {
                log.Fatalf("Couldn't execute: %v", err)
            }
        case "send":
            sendMessage(c, os.Args[2], os.Args[3])
            if err != nil {
                log.Fatalf("Couldn't execute: %v", err)
            }
        case "stream":
            getMessage(c, os.Args[2])
            if err != nil {
                log.Fatalf("Couldn't execute: %v", err)
            }
        case "chat":
